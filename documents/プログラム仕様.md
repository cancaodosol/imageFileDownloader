...
- ダウンロードファイル: `downloads/`
  - ファイル名ルール:
    ...
    - 禁止文字（`\\ / : * ? " < > |`）は `_` に置換
    - 同名が既にある場合は連番を付与（`photo.jpg`, `photo_1.jpg`, `photo_2.jpg` ...）
    - 拡張子はURL優先。無い場合は Content-Type から推定し、不明なら `.bin`
- ログ出力: `logs/`
  - `logs/success.log`: ダウンロード成功したURLを1行ずつ追記（再実行時のスキップ判定に使用）。
  - `logs/failed.log`: ダウンロード失敗したURLとエラー内容を1行で記録。

## 動作概要
1. `target.csv` が無ければ終了。
2. `logs/success.log` を読み込み、成功済みURLを集合化。
3. CSVを先頭から処理し、各行で以下を出し分ける:
   - 成功済みURL: `[SKIP] {名称} <- {URL}`
   - 新規URL・成功: ダウンロードして `[OK] {名称} -> {保存パス}`、`success.log` にURL追記
   - 新規URL・失敗: `[NG] {名称} <- {URL} ({エラー})`、`failed.log` にURLとエラー記録
